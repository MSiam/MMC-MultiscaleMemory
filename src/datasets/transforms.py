...
from src.util.box_ops import box_xyxy_to_cxcywh, box_iou
...
def crop(clip, target, region):
    ...
    if "masks" in target:
        ...
        fields.append("masks")
    # import ipdb; ipdb.set_trace()
    if 'soenet_feats' in target:
        soenet_feats = target['soenet_feats']
        if type(soenet_feats) == list:
            num_feats = len(soenet_feats)
            for layer_id in range(num_feats):
                # import ipdb; ipdb.set_trace()
                feat_i = target['soenet_feats'][layer_id]
                tt, ch, hh, ww = feat_i.shape
                i_lr = math.floor((float(i) / o_h) * hh)
                j_lr = math.floor((float(j) / o_w) * ww)
                h_lr = math.floor((float(h) / o_h) * hh)
                w_lr = math.floor((float(w) / o_w) * ww)
                # logger.debug('%d-%d-%d-%d->%d-%d-%d-%d' % (i, j, h, w, i_lr, j_lr, h_lr, w_lr))
                feat_i_crop = feat_i[:, :, i_lr:i_lr + h_lr, j_lr: j_lr + w_lr]
                target['soenet_feats'][layer_id] = feat_i_crop
        else:
            tt, ch, hh, ww = soenet_feats.shape
            i_lr = math.floor((float(i) / o_h) * hh)
            j_lr = math.floor((float(j) / o_w) * ww)
            h_lr = math.floor((float(h) / o_h) * hh)
            w_lr = math.floor((float(w) / o_w) * ww)
            # logger.debug('%d-%d-%d-%d->%d-%d-%d-%d' % (i, j, h, w, i_lr, j_lr, h_lr, w_lr))
            soenet_feats = soenet_feats[:, :, i_lr:i_lr + h_lr, j_lr: j_lr + w_lr]
            target['soenet_feats'] = soenet_feats
    ...
    if 'soenet_feats' in target:
        ...
        if type(soenet_feats) == list:
            ...
            for i in range(num_feats):
                target['soenet_feats'][i] = F.hflip(target['soenet_feats'][i])
        else:
            target['soenet_feats'] = F.hflip(target['soenet_feats'])
